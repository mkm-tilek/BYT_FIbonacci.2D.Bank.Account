...
public class Bank {
	 ...
	 */
	public boolean transfer(String fromaccount, Bank tobank, String toaccount, Money amount) throws AccountDoesNotExistException {
		if (!accountlist.containsKey(fromaccount) || !tobank.accountlist.containsKey(toaccount)) {
			throw new AccountDoesNotExistException();
		} else if (fromaccount.equals(toaccount)) {
			return false;
		} else {
			if (getBalance(fromaccount) >= amount.getAmount()) {
	...
	public void tick(int period) throws AccountDoesNotExistException {
		int count = 0;
		while (period > count) {
			for (Account account : accountlist.values()) {
				account.tick();
